...
public class TestRngState {
  ...
  void testUnfold() {
    ...
    Assertions.assertTrue(listIntEqual.eq(s.take(size).toList(), expected1));
  }
  ...
  void testTransitions() {
    ...
    Assertions.assertTrue(listIntEqual.eq(ints, expected1));
  }

  @Test
  void testSequence() {
    List<Integer> list = State.sequence(List.replicate(size, defaultState())).eval(defaultRng());
    Assertions.assertTrue(listIntEqual.eq(list, expected1));
  }
...
//        System.out.println(list.toString());
    ...
    Assertions.assertTrue(listIntEqual.eq(list, expected));
  }
