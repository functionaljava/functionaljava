...
public abstract class Stream<A> implements Iterable<A> {
	...
	/**
	 * Returns an iterator for this stream. This method exists to permit the use
	 * in a <code>for</code>-each loop.
	 * 
	 ...
	 * 
	 * @return The stream without the first element or fails for the empty
	 *         stream.
	 */
	public abstract P1<Stream<A>> tail();

	/**
	 * Returns <code>true</code> if this stream is empty, <code>false</code>
	 * otherwise.
	 * 
	 * @return <code>true</code> if this stream is empty, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Returns <code>false</code> if this stream is empty, <code>true</code>
	 * otherwise.
	 * 
	 * @return <code>false</code> if this stream is empty, <code>true</code>
	 *         otherwise.
	 */
	 ...
	 * 
	 * @param nil
	 *            The value to return if this stream is empty.
	 * @param cons
	 *            The function to apply to the head and tail of this stream if
	 *            it is not empty.
	...
	/**
	 * Performs a right-fold reduction across this stream. This function uses
	 * O(length) stack space.
	 * 
	 * @param f
	 *            The function to apply on each element of the stream.
	 * @param b
	...
	public final <B> B foldRight1(final F<A, F<B, B>> f, final B b) {
		return foldRight(
				compose(Function.<P1<B>, B, B> andThen().f(P1.<B> __1()), f), b);
	}
	...
	/**
	 * Performs a left-fold reduction across this stream. This function runs in
	 * constant space.
	 * 
	 * @param f
	 *            The function to apply on each element of the stream.
	...
	/**
	 * Takes the first 2 elements of the stream and applies the function to
	 * them, then applies the function to the result and the third element and
	 * so on.
	 * 
	 * @param f
	 *            The function to apply on each element of the stream.
	...
	/**
	 * Returns the head of this stream if there is one or the given argument if
	 * this stream is empty.
	 * 
	 * @param a
	 *            The argument to return if this stream is empty.
	 * @return The head of this stream if there is one or the given argument if
	 *         this stream is empty.
	 */
	...
	/**
	 * Returns the tail of this stream if there is one or the given argument if
	 * this stream is empty.
	 * 
	 * @param as
	 *            The argument to return if this stream is empty.
	 * @return The tail of this stream if there is one or the given argument if
	 *         this stream is empty.
	 */
	 ...
	 * 
	 * @param a
	 *            The value to intersperse between values of the stream.
	 * @return A new stream with the given value between each two elements of
	 *         the stream.
	 */
	public final Stream<A> intersperse(final A a) {
		return isEmpty() ? this : cons(head(), new P1<Stream<A>>() {
			...
			public Stream<A> prefix(final A x, final Stream<A> xs) {
				return xs.isEmpty() ? xs : cons(x,
						p(cons(xs.head(), new P1<Stream<A>>() {
							public Stream<A> _1() {
	 ...
	 * 
	 * @param f
	 *            The function to map across this stream.
	 * @return A new stream after the given function has been applied to each
	 *         element.
	 */
	public final <B> Stream<B> map(final F<A, B> f) {
		return isEmpty() ? Stream.<B> nil() : cons(f.f(head()),
				new P1<Stream<B>>() {
					public Stream<B> _1() {
	...
	/**
	 * Filters elements from this stream by returning only elements which
	 * produce <code>true</code> when the given function is applied to them.
	 * 
	 * @param f
	 ...
	 * 
	 * @param as
	 *            The stream to append to this one.
	...
	/**
	 * Returns a new stream of all the items in this stream that do not appear
	 * in the given stream.
	 * 
	 * @param eq
	 *            an equality for the items of the streams.
	 * @param xs
	 *            a list to subtract from this stream.
	 * @return a stream of all the items in this stream that do not appear in
	 *         the given stream.
	 */
	public final Stream<A> minus(final Equal<A> eq, final Stream<A> xs) {
		return removeAll(compose(Monoid.disjunctionMonoid.sumLeftS(),
				xs.mapM(curry(eq.eq()))));
	}

	/**
	 * Filters elements from this stream by returning only elements which
	 * produce <code>false</code> when the given function is applied to them.
	 * 
	 * @param f
	 *            The predicate function to filter on.
	 ...
	 * 
	 * @param fs
	 *            The stream of functions to sequence into a single function
	 *            that returns a stream.
	 * @return A function that, when given an argument, applies all the
	 *         functions in the given stream to it and returns a stream of the
	 *         results.
	 */
	public static <A, B> F<B, Stream<A>> sequence_(final Stream<F<B, A>> fs) {
		return fs.foldRight(
				new F2<F<B, A>, P1<F<B, Stream<A>>>, F<B, Stream<A>>>() {
					public F<B, Stream<A>> f(final F<B, A> baf,
							final P1<F<B, Stream<A>>> p1) {
						return Function.bind(baf, p1._1(), Function
								.curry(new F2<A, Stream<A>, Stream<A>>() {
	...
	/**
	 * Maps the given function of arity-2 across this stream and returns a
	 * function that applies all the resulting functions to a given argument.
	 * 
	 * @param f
	 *            A function of arity-2
	 * @return A function that, when given an argument, applies the given
	 *         function to that argument and every element in this list.
	 */
	...
	/**
	 * Binds the given function across each element of this stream with a final
	 * join.
	 * 
	 * @param f
	 *            The function to apply to each element of this stream.
	...
	/**
	 * Binds the given function across each element of this stream and the given
	 * stream with a final join.
	 * 
	 * @param sb
	 *            A given stream to bind the given function with.
	 * @param f
	 *            The function to apply to each element of this stream and the
	 *            given stream.
	 * @return A new stream after performing the map, then final join.
	 */
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D> Stream<D> bind(final Stream<B> sb,
			final Stream<C> sc, final F<A, F<B, F<C, D>>> f) {
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D, E> Stream<E> bind(final Stream<B> sb,
			final Stream<C> sc, final Stream<D> sd,
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D, E, F$> Stream<F$> bind(final Stream<B> sb,
			final Stream<C> sc, final Stream<D> sd, final Stream<E> se,
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D, E, F$, G> Stream<G> bind(final Stream<B> sb,
			final Stream<C> sc, final Stream<D> sd, final Stream<E> se,
			final Stream<F$> sf, final F<A, F<B, F<C, F<D, F<E, F<F$, G>>>>>> f) {
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D, E, F$, G, H> Stream<H> bind(final Stream<B> sb,
			final Stream<C> sc, final Stream<D> sd, final Stream<E> se,
			final Stream<F$> sf, final Stream<G> sg,
	 ...
	 * @return A new stream after performing the map, then final join.
	 */
	public final <B, C, D, E, F$, G, H, I> Stream<I> bind(final Stream<B> sb,
			final Stream<C> sc, final Stream<D> sd, final Stream<E> se,
			final Stream<F$> sf, final Stream<G> sg, final Stream<H> sh,
			final F<A, F<B, F<C, F<D, F<E, F<F$, F<G, F<H, I>>>>>>>> f) {
		return sh.apply(bind(sb, sc, sd, se, sf, sg, f));
	}

	/**
	 * Performs a bind across each stream element, but ignores the element value
	 * each time.
	 * 
	 * @param bs
	 *            The stream to apply in the final join.
	 ...
	 * 
	 * @param sf
	 *            The stream of functions to apply.
	 ...
	 * 
	 * @param as
	 *            The stream to interleave this stream with.
	 * @return A new stream with elements interleaved from this stream and the
	 *         given stream.
	 */
	public final Stream<A> interleave(final Stream<A> as) {
		return isEmpty() ? as : as.isEmpty() ? this : cons(head(),
				new P1<Stream<A>>() {
					@Override
					public Stream<A> _1() {
	 ...
	 * @return A new stream with the elements of this stream sorted according to
	 *         the given ordering.
	 */
	...
	// Merges a stream of individually sorted streams into a single sorted
	// stream.
	private static <A> Stream<A> mergesort(final Ord<A> o,
			final Stream<Stream<A>> s) {
		if (s.isEmpty())
			return nil();
	...
	private static <A> Stream<Stream<A>> mergePairs(final Ord<A> o,
			final Stream<Stream<A>> s) {
	...
	private static <A> Stream<A> merge(final Ord<A> o, final Stream<A> xs,
			final Stream<A> ys) {
		if (xs.isEmpty())
			return ys;
		if (ys.isEmpty())
	...
	/**
	 * Sort this stream according to the given ordering, using a parallel Quick
	 * Sort algorithm that uses the given parallelisation strategy.
	 * 
	 * @param o
	 *            An ordering for the elements of this stream.
	 * @param s
	...
	private Promise<Stream<A>> qs(final Ord<A> o, final Strategy<Unit> s) {
		if (isEmpty())
			...
			final Promise<Stream<A>> left = Promise.join(s,
					xs.map(flt(o, s, x, id)));
	...
	private static <A> F<Stream<A>, Promise<Stream<A>>> flt(final Ord<A> o,
			final Strategy<Unit> s, final A x, final F<Boolean, Boolean> f) {
	...
	/**
	 * Returns a stream of integers from the given <code>from</code> value
	 * (inclusive) to the given <code>to</code> value (exclusive).
	 * 
	 * @param from
	 *            The minimum value for the stream (inclusive).
	 * @param to
	 *            The maximum value for the stream (exclusive).
	 * @return A stream of integers from the given <code>from</code> value
	 *         (inclusive) to the given <code>to</code> value (exclusive).
	 */
	public static Stream<Integer> range(final int from, final long to) {
		return from >= to ? Stream.<Integer> nil() : cons(from,
				new P1<Stream<Integer>>() {
	 ...
	 * 
	 * @param as
	 *            The elements which which to construct a stream.
	 ...
	 * increments of <code>1</code>.
	 * 
	 * @param e
	 *            The enumerator to compute successors from.
	 * @param from
	 *            The value to begin computing successors from.
	 * @return A stream that is either infinite or bounded up to the maximum
	 *         value of the given iterator starting at the given value and
	 *         stepping at increments of <code>1</code>.
	 */
	 ...
	 *            The value to begin computing successors from.
	 * @param step
	 *            The increment to step.
	 * @return A stream that is either infinite or bounded up to the maximum
	 *         value of the given iterator starting at the given value and
	 *         stepping at the given increment.
	 */
	public static <A> Stream<A> forever(final Enumerator<A> e, final A from,
			final long step) {
	...
	/**
	 * Returns a stream using the given enumerator from the given value to the
	 * other given value stepping at increments of <code>1</code>.
	 * 
	 * @param e
	 *            The enumerator to compute successors from.
	 * @param from
	 *            The value to begin computing successors from.
	 * @param to
	 *            The value to stop computing successors from.
	 * @return A stream using the given enumerator from the given value to the
	 *         other given value stepping at increments of <code>1</code>.
	 */
	public static <A> Stream<A> range(final Enumerator<A> e, final A from,
			final A to) {
	...
	/**
	 * Returns a stream using the given enumerator from the given value to the
	 * other given value stepping at the given increment.
	 * 
	 * @param e
	 *            The enumerator to compute successors from.
	 * @param from
	 ...
	 */
	public static <A> Stream<A> range(final Enumerator<A> e, final A from,
			final A to, final long step) {
		final Ordering o = e.order().compare(from, to);
		return o == EQ || step > 0L && o == GT || step < 0L && o == LT ? single(from)
				: cons(from, new P1<Stream<A>>() {
					public Stream<A> _1() {
						return Stream.join(e.plus(from, step)
								.filter(new F<A, Boolean>() {
									public Boolean f(final A a) {
										return !(o == LT ? e.order()
												.isLessThan(to, a) : e.order()
												.isGreaterThan(to, a));
									}
	...
	/**
	 * Returns an infinite stream of integers from the given <code>from</code>
	 * value (inclusive).
	 * 
	 * @param from
	 *            The minimum value for the stream (inclusive).
	...
	/**
	 * Zips this stream with the given stream of functions, applying each
	 * function in turn to the corresponding element in this stream to produce a
	 * new stream. If this stream and the given stream have different lengths,
	 * then the longer stream is normalised so this function never fails.
	 * 
	 * @param fs
	 *            The stream of functions to apply to this stream.
	 * @return A new stream with a length the same as the shortest of this
	 *         stream and the given stream.
	 */
	public final <B> Stream<B> zapp(final Stream<F<A, B>> fs) {
		return fs.isEmpty() || isEmpty() ? Stream.<B> nil() : cons(
				fs.head().f(head()), new P1<Stream<B>>() {
					public Stream<B> _1() {
	...
	/**
	 * Zips this stream with the given stream using the given function to
	 * produce a new stream. If this stream and the given stream have different
	 * lengths, then the longer stream is normalised so this function never
	 * fails.
	 * 
	 * @param bs
	 *            The stream to zip this stream with.
	 * @param f
	 *            The function to zip this stream and the given stream with.
	 * @return A new stream with a length the same as the shortest of this
	 *         stream and the given stream.
	 */
	public final <B, C> Stream<C> zipWith(final Stream<B> bs,
			final F<A, F<B, C>> f) {
	...
	/**
	 * Partially-applied version of zipWith. Returns a function that zips a
	 * given stream with this stream using the given function.
	 * 
	 * @param f
	 *            The function to zip this stream and a given stream with.
	 * @return A function that zips a given stream with this stream using the
	 *         given function.
	 */
	...
	/**
	 * Zips this stream with the given stream to produce a stream of pairs. If
	 * this stream and the given stream have different lengths, then the longer
	 * stream is normalised so this function never fails.
	 * 
	 * @param bs
	 *            The stream to zip this stream with.
	 * @return A new stream with a length the same as the shortest of this
	 ...
	 * <code>Left</code> if empty, or the first element in <code>Right</code>.
	 * 
	 * @param x
	 *            The value to return in left if this stream is empty.
	 * @return An either projection of this stream.
	 */
	public final <X> Either<X, A> toEither(final P1<X> x) {
		return isEmpty() ? Either.<X, A> left(x._1()) : Either
				.<X, A> right(head());
	}

	/**
	 * Returns an option projection of this stream; <code>None</code> if empty,
	 * or the first element in <code>Some</code>.
	 * 
	...
	public final Array<A> toArray(final Class<A[]> c) {
		final A[] a = (A[]) java.lang.reflect.Array.newInstance(
				c.getComponentType(), length());
	...
	/**
	 * Returns a string from the given stream of characters. The inverse of this
	 * function is {@link #fromString(String)}.
	 * 
	 * @param cs
	 *            The stream of characters to produce the string from.
	...
	/**
	 * Returns a stream of characters from the given string. The inverse of this
	 * function is {@link #asString(Stream)}.
	 * 
	 * @param s
	 *            The string to produce the stream of characters from.
	...
	/**
	 * Drops the given number of elements from the head of this stream if they
	 * are available.
	 * 
	 * @param i
	 *            The number of elements to drop from the head of this stream.
	...
	/**
	 * Returns the first elements of the head of this stream that match the
	 * given predicate function.
	 * 
	 * @param f
	 *            The predicate function to apply on this stream until it finds
	 *            an element that does not hold, or the stream is exhausted.
	 * @return The first elements of the head of this stream that match the
	 *         given predicate function.
	 */
	public final Stream<A> takeWhile(final F<A, Boolean> f) {
		return isEmpty() ? this : f.f(head()) ? cons(head(),
				new P1<Stream<A>>() {
					public Stream<A> _1() {
	...
	/**
	 * Removes elements from the head of this stream that do not match the given
	 * predicate function until an element is found that does match or the
	 * stream is exhausted.
	 * 
	 * @param f
	 *            The predicate function to apply through this stream.
	 * @return The stream whose first element does not match the given predicate
	 *         function.
	 */
	...
	/**
	 * Returns a tuple where the first element is the longest prefix of this
	 * stream that satisfies the given predicate and the second element is the
	 * remainder of the stream.
	 * 
	 * @param p
	 *            A predicate to be satisfied by a prefix of this stream.
	 * @return A tuple where the first element is the longest prefix of this
	 *         stream that satisfies the given predicate and the second element
	...
	public final P2<Stream<A>, Stream<A>> span(final F<A, Boolean> p) {
		if (isEmpty())
			...
			return new P2<Stream<A>, Stream<A>>() {
				...
				public Stream<A> _1() {
					return cons(head(),
							yszs.map(P2.<Stream<A>, Stream<A>> __1()));
				}
	...
	/**
	 * Returns a new stream resulting from replacing all elements that match the
	 * given predicate with the given element.
	 * 
	 * @param p
	 *            The predicate to match replaced elements.
	 * @param a
	 *            The element with which to replace elements.
	 * @return A new stream resulting from replacing all elements that match the
	 *         given predicate with the given element.
	 */
	...
	/**
	 * Returns a tuple where the first element is the longest prefix of this
	 * stream that does not satisfy the given predicate and the second element
	 * is the remainder of the stream.
	 * 
	 * @param p
	 *            A predicate not to be satisfied by a prefix of this stream.
	 * @return A tuple where the first element is the longest prefix of this
	 *         stream that does not satisfy the given predicate and the second
	 *         element is the remainder of the stream.
	 */
	...
	/**
	 * The length of this stream. This function will not terminate for an
	 * infinite stream.
	 * 
	 * @return The length of this stream.
	 */
	public final int length() {
		// we're using an iterative approach here as the previous implementation
		// (toList().length()) took
	...
	/**
	 * Returns <code>true</code> if the predicate holds for all of the elements
	 * of this stream, <code>false</code> otherwise (<code>true</code> for the
	 * empty stream).
	 * 
	 * @param f
	 *            the predicate function to test on each element of this stream.
	 * @return <code>true</code> if the predicate holds for all of the elements
	 *         of this stream, <code>false</code> otherwise.
	 */
	...
	/**
	 * Returns <code>true</code> if the predicate holds for at least one of the
	 * elements of this stream, <code>false</code> otherwise (<code>false</code>
	 * for the empty stream).
	 * 
	 * @param f
	 *            The predicate function to test on the elements of this stream.
	 * @return <code>true</code> if the predicate holds for at least one of the
	...
	/**
	 * Finds the first occurrence of an element that matches the given predicate
	 * or no value if no elements match.
	 * 
	 * @param f
	 *            The predicate function to test on elements of this stream.
	 * @return The first occurrence of an element that matches the given
	 *         predicate or no value if no elements match.
	 */
	 ...
	 * 
	 * @param k
	 *            A function to bind across this stream and its substreams.
	 * @return a new stream of the results of applying the given function to
	 *         this stream and its substreams.
	 */
	...
	/**
	 * Returns a stream of the suffixes of this stream. A stream is considered
	 * to be a suffix of itself in this context.
	 * 
	 * @return a stream of the suffixes of this stream, starting with the stream
	 *         itself.
	 */
	public final Stream<Stream<A>> tails() {
		return isEmpty() ? Stream.<Stream<A>> nil() : cons(this,
				new P1<Stream<Stream<A>>>() {
					public Stream<Stream<A>> _1() {
	...
	/**
	 * Returns a stream of all prefixes of this stream. A stream is considered a
	 * prefix of itself in tnis context.
	 * 
	 * @return a stream of the prefixes of this stream, starting with the stream
	 *         itself.
	 */
	public final Stream<Stream<A>> inits() {
		final Stream<Stream<A>> nil = Stream.cons(Stream.<A> nil(),
				new P1<Stream<Stream<A>>>() {
					public Stream<Stream<A>> _1() {
						return nil();
					}
	...
	/**
	 * Returns a stream of all infixes of this stream. A stream is considered to
	 * contain itself.
	 * 
	...
	/**
	 * Returns the position of the first element matching the given predicate,
	 * if any.
	 * 
	 * @param p
	 *            A predicate to match.
	 * @return the position of the first element matching the given predicate,
	 *         if any.
	 */
	...
	/**
	 * Applies a stream of comonadic functions to this stream, returning a
	 * stream of values.
	 * 
	 * @param fs
	 *            A stream of comonadic functions to apply to this stream.
	 * @return A new stream of the results of applying the stream of functions
	 *         to this stream.
	 */
	public final <B> Stream<B> sequenceW(final Stream<F<Stream<A>, B>> fs) {
		return fs.isEmpty() ? Stream.<B> nil() : cons(fs.head().f(this),
				new P1<Stream<B>>() {
					public Stream<B> _1() {
	 ...
	 * 
	 * @return A function from natural numbers to values with the corresponding
	 ...
	 * 
	 * @param f
	 *            The function to convert to a stream.
	 * @return A new stream of the results of the given function applied to the
	...
	/**
	 * Converts a function of an enumerable type to a stream of the results of
	 * that function, starting at the given index.
	 * 
	 * @param e
	 *            An enumerator for the domain of the function.
	 * @param f
	 *            The function to convert to a stream.
	 * @param i
	 *            The index into the function at which to begin the stream.
	 * @return A new stream of the results of the given function applied to the
	 *         values of the given enumerator, starting at the given value.
	 */
	public static <A, B> Stream<A> fromFunction(final Enumerator<B> e,
			final F<B, A> f, final B i) {
		return cons(f.f(i), new P1<Stream<A>>() {
			public Stream<A> _1() {
				final Option<B> s = e.successor(i);
				return s.isSome() ? fromFunction(e, f, s.some()) : Stream
						.<A> nil();
			}
		});
	}
	...
	/**
	 * Transforms a stream of pairs into a stream of first components and a
	 * stream of second components.
	 * 
	 * @param xs
	 *            The stream of pairs to transform.
	 * @return A stream of first components and a stream of second components.
	 */
	public static <A, B> P2<Stream<A>, Stream<B>> unzip(
			final Stream<P2<A, B>> xs) {
		return xs
				.foldRight(
						new F2<P2<A, B>, P1<P2<Stream<A>, Stream<B>>>, P2<Stream<A>, Stream<B>>>() {
							public P2<Stream<A>, Stream<B>> f(final P2<A, B> p,
									final P1<P2<Stream<A>, Stream<B>>> ps) {
								final P2<Stream<A>, Stream<B>> pp = ps._1();
								return P.p(cons(p._1(), P.p(pp._1())),
										cons(p._2(), P.p(pp._2())));
							}
	...
	/**
	 * Returns a function that prepends (cons) an element to a stream to produce
	 * a new stream.
	 * 
	 * @return A function that prepends (cons) an element to a stream to produce
	 *         a new stream.
	 */
	...
	/**
	 * Prepends the given head element to the given tail element to produce a
	 * new stream.
	 * 
	 * @param head
	 *            The element to prepend.
	 * @param tail
	 *            The stream to prepend to.
	...
	/**
	 * Unfolds across the given function starting at the given value to produce
	 * a stream.
	 * 
	 * @param f
	 ...
	 * @return A new stream that is a result of unfolding until the function
	 *         does not produce a value.
	 */
	public static <A, B> Stream<A> unfold(final F<B, Option<P2<A, B>>> f,
			final B b) {
	...
	/**
	 * Creates a stream where the first item is calculated by applying the
	 * function on the third argument, the second item by applying the function
	 * on the previous result and so on.
	 * 
	 * @param f
	 *            The function to iterate with.
	 * @param p
	 *            The predicate which must be true for the next item in order to
	 *            continue the iteration.
	 * @param a
	 *            The input to the first iteration.
	 * @return A stream where the first item is calculated by applying the
	 *         function on the third argument, the second item by applying the
	 ...
	 * 
	 * @param i
	 *            The iterable to take to a stream.
	 ...
	 * 
	 * @param a
	 *            The element to repeat infinitely.
	...
	/**
	 * Returns an infinite-length stream of the given elements cycling. Fails on
	 * the empty stream.
	 * 
	 * @param as
	 *            The elements to cycle infinitely. This must not be empty.
	...
	/**
	 * Returns a stream constructed by applying the given iteration function
	 ...
	 * @return A stream constructed by applying the given iteration function
	 *         starting at the given value.
	 */
	 ...
	 * 
	 * @return A function that returns a stream constructed by applying a given
	 *         iteration function starting at a given value.
	 */
	 ...
	 * 
	 * @return A function that binds a given function across a given stream,
