...
public abstract class HList<A extends HList<A>> {
	...
	/**
	 * Extends (cons) this list by prepending the given element, returning a new
	 * list.
	 * 
	 * @param e
	 *            an element to prepend to this list.
	 * @return a new heterogeneous list, consisting of the given element
	...
	public static final class HAppend<A, B, C> {
		 ...
		 * @return a new heterogeneous list consisting of the second argument
		...
		/**
		 * Returns a method for appending lists to a nonempty heterogeneous
		 * list.
		 * 
		 * @param h
		 *            a method for appending lists to the tail of the given
		 *            nonempty list.
		 * @return a method for appending lists to a nonempty heterogeneous
		 *         list.
		 */
		public static <X, A extends HList<A>, B, C extends HList<C>, H extends HAppend<A, B, C>> HAppend<HCons<X, A>, B, HCons<X, C>> append(
				final H h) {
			return new HAppend<HCons<X, A>, B, HCons<X, C>>((c, l) -> cons(
					c.head(), h.append(c.tail(), l)));
		}
	}
	...
	public abstract static class Apply<F$, A, R> {
		 ...
		 * 
		 * @return an operator that applies a given function to a given
		 *         argument.
		 */
		 ...
		 * Identity operator
		 * 
		 * @return An operator that returns its second argument no matter which
		 ...
		 *            The domain.
		 * @param <Y>
		 *            The type through which to compose.
		 * @param <Z>
		 *            The codomain.
		...
		/**
		 * A function application operator for concatenating heterogeneous
		 * lists.
		 * 
		 * @param <A>
		 *            The type of the list to which to append.
		 * @param <B>
		 *            The type of the list to append.
		 * @param <C>
	 ...
	 * 
	 * @param <G>
	 *            The type of the function with which to fold.
	 * @param <V>
	 *            The type of the value to be substituted for the empty list.
	 * @param <L>
	 *            The type of the heterogeneous list to be folded.
	 * @param <R>
	...
	public static final class HFoldr<G, V, L, R> {
		 ...
		 * 
		 * @param p
		 *            An operator that applies a function on the head of the
		 *            list and the fold of its tail.
		 * @param h
		 *            A fold instance for the tail of the list.
		 * @param <E>
		 *            The type of the head of the list.
		 * @param <G>
		 *            The type of function to apply to the head of the list and
		 *            the fold of its tail.
		 * @param <V>
		 *            The type of value to substitute for the empty list.
		 * @param <L>
		 *            The type of the tail of the list.
		 * @param <R>
		 *            The type of the fold of the tail of the list.
		 * @param <RR>
		 *            The return type of the fold.
		 * @param <H>
		 *            The type of the fold instance for the tail of the list.
		 * @param <PP>
		 *            The type of the given function application operator.
		 * @return A fold instance for a non-empty heterogeneous list.
		 */
		public static <E, G, V, L extends HList<L>, R, RR, H extends HFoldr<G, V, L, R>, PP extends Apply<G, P2<E, R>, RR>> HFoldr<G, V, HCons<E, L>, RR> hFoldr(
				final PP p, final H h) {
			return new HFoldr<G, V, HCons<E, L>, RR>(
					new F3<G, V, HCons<E, L>, RR>() {
						public RR f(final G f, final V v, final HCons<E, L> c) {
							return p.apply(f,
									P.p(c.head(), h.foldRight(f, v, c.tail())));
						}
					});
		}
		 ...
		 * 
		 * @param f
		 *            A function with which to fold.
		 * @param v
		 *            The value to substitute for the empty list.
		 * @param l
		 *            The heterogeneous list to be folded.
		 * @return a value obtained by folding the given list with the given
		 *         function.
		 */
	 ...
	 * The nonempty list
	 */
	public static final class HCons<E, L extends HList<L>> extends
			HList<HCons<E, L>> {
		private final E e;
		private final L l;
