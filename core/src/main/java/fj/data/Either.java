 ...
 * (a disjoint union). The data constructors; <code>Left</code> and
 * <code>Right</code> represent the two possible values. The <code>Either</code>
 * type is often used as an alternative to <code>scala.Option</code> where
 * <code>Left</code> represents failure (by convention) and <code>Right</code>
 * is akin to <code>Some</code>.
 * 
...
public abstract class Either<A, B> {
	...
	public final <X> X either(final F<A, X> left, final F<B, X> right) {
		return isLeft() ? left.f(left().value()) : right.f(right().value());
	}
	...
	public final Either<B, A> swap() {
		return isLeft() ? new Right<B, A>(((Left<A, B>) this).a)
				: new Left<B, A>(((Right<A, B>) this).b);
	}
	...
	public final class LeftProjection<A, B> implements Iterable<A> {
		...
		/**
		 * Returns the value of this projection or fails with the given error
		 * message.
		 * 
		 * @param err
		...
		/**
		 * The value of this projection or fails with a specialised error
		 * message.
		 * 
		 * @return The value of this projection.
		 */
		...
		/**
		 * The value of this projection or the result of the given function on
		 * the opposing projection's value.
		 * 
		 * @param f
		 *            The function to execute if this projection has no value.
		 * @return The value of this projection or the result of the given
		 *         function on the opposing projection's value.
		 */
		...
		public <X> Either<X, B> map(final F<A, X> f) {
			return isLeft() ? new Left<X, B>(f.f(value())) : new Right<X, B>(e
					.right().value());
		}

		/**
		 * Binds the given function across this projection's value if it has
		 * one.
		 * 
		 * @param f
		...
		/**
		 * Returns <code>None</code> if this projection has no value or if the
		 * given predicate <code>p</code> does not hold for the value,
		 * otherwise, returns a right in <code>Some</code>.
		 * 
		 * @param f
		 *            The predicate function to test on this projection's value.
		 * @return <code>None</code> if this projection has no value or if the
		 *         given predicate <code>p</code> does not hold for the value,
		 *         otherwise, returns a right in <code>Some</code>.
		 */
		public <X> Option<Either<A, X>> filter(final F<A, Boolean> f) {
			return isLeft() ? f.f(value()) ? Option
					.<Either<A, X>> some(new Left<A, X>(value())) : Option
					.<Either<A, X>> none() : Option.<Either<A, X>> none();
		}
		...
		public <X> Either<X, B> apply(final Either<F<A, X>, B> e) {
			return e.left().bind((F<A, X> f) -> map(f));
		}

		/**
		 * Returns <code>true</code> if no value or returns the result of the
		...
		/**
		 * Returns <code>false</code> if no value or returns the result of the
		 * application of the given function to the value.
		 * 
		 * @param f
		 *            The predicate function to test on this projection's value.
		 * @return <code>false</code> if no value or returns the result of the
		 *         application of the given function to the value.
		 */
		...
		/**
		 * Returns a single element list if this projection has a value,
		 * otherwise an empty list.
		 * 
		 * @return A single element list if this projection has a value,
		 *         otherwise an empty list.
		 */
		...
		/**
		 * Returns a single element array if this projection has a value,
		 * otherwise an empty array.
		 * 
		 * @return A single element array if this projection has a value,
		 *         otherwise an empty array.
		 */
		...
		/**
		 * Returns a single element stream if this projection has a value,
		 * otherwise an empty stream.
		 * 
		 * @return A single element stream if this projection has a value,
		 *         otherwise an empty stream.
		 */
	...
	public final class RightProjection<A, B> implements Iterable<B> {
		...
		public <X> Either<A, X> map(final F<B, X> f) {
			return isRight() ? new Right<A, X>(f.f(value())) : new Left<A, X>(e
					.left().value());
		}
		 ...
		 * Returns <code>None</code> if this projection has no value or if the
		 * given predicate <code>p</code> does not hold for the value,
		 * otherwise, returns a left in <code>Some</code>.
		 * 
		 * @param f
		 *            The predicate function to test on this projection's value.
		 * @return <code>None</code> if this projection has no value or if the
		...
		public <X> Option<Either<X, B>> filter(final F<B, Boolean> f) {
			return isRight() ? f.f(value()) ? Option
					.<Either<X, B>> some(new Right<X, B>(value())) : Option
					.<Either<X, B>> none() : Option.<Either<X, B>> none();
		}
		...
		public <X> Either<A, X> apply(final Either<A, F<B, X>> e) {
			return e.right().bind((F<B, X> f) -> map(f));
		}
	...
	/**
	 * @return A function that maps another function across an either's left
	 *         projection.
	 */
	...
	/**
	 * @return A function that maps another function across an either's right
	 *         projection.
	 */
	...
	/**
	 * Sequences through the left side of the either monad with a list of
	 * values.
	 * 
	 * @param a
	 *            The list of values to sequence with the either monad.
	 * @return A sequenced value.
	 */
	public static <A, X> Either<List<A>, X> sequenceLeft(
			final List<Either<A, X>> a) {
		return a.isEmpty() ? Either.<List<A>, X> left(List.<A> nil()) : a
				.head().left()
				.bind((A aa) -> sequenceLeft(a.tail()).left().map(cons_(aa)));
	}

	/**
	 * Sequences through the right side of the either monad with a list of
	 * values.
	 * 
	 * @param a
	 *            The list of values to sequence with the either monad.
	 * @return A sequenced value.
	 */
	public static <B, X> Either<X, List<B>> sequenceRight(
			final List<Either<X, B>> a) {
		return a.isEmpty() ? Either.<X, List<B>> right(List.<B> nil()) : a
				.head().right()
				.bind((B bb) -> sequenceRight(a.tail()).right().map(cons_(bb)));
	}
	 ...
	 * @return An <code>Either</code> to its contained value within left or
	 *         right.
	 */
	...
	/**
	 * If the condition satisfies, return the given A in left, otherwise, return
	 * the given B in right.
	 * 
	 * @param c
	 *            The condition to test.
	 * @param right
	 *            The right value to use if the condition satisfies.
	 * @param left
	 *            The left value to use if the condition does not satisfy.
	 * @return A constructed either based on the given condition.
	 */
	public static <A, B> Either<A, B> iif(final boolean c, final P1<B> right,
			final P1<A> left) {
	...
	public static <A, B> List<A> lefts(final List<Either<A, B>> es) {
		return es.foldRight(e -> as -> e.isLeft() ? as.cons(e.left().value())
				: as, List.<A> nil());
	}
