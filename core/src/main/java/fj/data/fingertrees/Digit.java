...
/**
 * A digit is a vector of 1-4 elements. Serves as a pointer to the prefix or
 * suffix of a finger tree.
 */
public abstract class Digit<V, A> {
	/**
	 * Folds this digit to the right using the given function and the given
	 * initial value.
	 * 
	 * @param f
	 ...
	 * @return The right reduction of this digit with the given function and the
	 *         given initial value.
	 */
	public abstract <B> B foldRight(final F<A, F<B, B>> f, final B z);

	/**
	 * Folds this digit to the left using the given function and the given
	 * initial value.
	 * 
	 * @param f
	 *            A function with which to fold this digit.
	 ...
	 * @return The left reduction of this digit with the given function and the
	 *         given initial value.
	 */
	...
	/**
	 * Maps a function across the elements of this digit, measuring with the
	 * given measurement.
	 * 
	 * @param f
	 *            A function to map across the elements of this digit.
	 * @param m
	 *            A measuring for the function's domain (destination type).
	 * @return A new digit with the same structure as this digit, but with all
	 *         elements transformed with the given function and measured with
	 *         the given measuring.
	 */
	public final <B> Digit<V, B> map(final F<A, B> f, final Measured<V, B> m) {
		return match(one -> new One<V, B>(m, f.f(one.value())),
	...
	/**
	 * Structural pattern matching on digits. Applies the function that matches
	 * the structure of this digit.
	 * 
	 * @param one
	 *            A function to apply to this digit if it's One.
	 * @param two
	 *            A function to apply to this digit if it's Two.
	 * @param three
	 ...
	 */
	public abstract <B> B match(final F<One<V, A>, B> one,
			final F<Two<V, A>, B> two, final F<Three<V, A>, B> three,
	...
	/**
	 * Returns the sum of the measurements of this digit according to the
	 * monoid.
	 * 
	 * @return the sum of the measurements of this digit according to the
	 *         monoid.
	 */
	public final V measure() {
		return foldLeft(Function.curry((v, a) -> m.sum(v, m.measure(a))),
				m.zero());
	}
	...
	public final FingerTree<V, A> toTree() {
		...
		return match(one -> mk.single(one.value()), two -> mk.deep(
				mk.one(two.values()._1()),
				new Empty<V, Node<V, A>>(m.nodeMeasured()),
				mk.one(two.values()._2())), three -> mk.deep(
				mk.two(three.values()._1(), three.values()._2()),
				new Empty<V, Node<V, A>>(m.nodeMeasured()),
				mk.one(three.values()._3())), four -> mk.deep(
				mk.two(four.values()._1(), four.values()._2()),
				new Empty<V, Node<V, A>>(m.nodeMeasured()),
