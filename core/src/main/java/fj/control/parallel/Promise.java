...
/**
 * Represents a non-blocking future value. Products, functions, and actors,
 * given to the methods on this class, are executed concurrently, and the
 * Promise serves as a handle on the result of the computation. Provides monadic
...
public final class Promise<A> {
	...
	private Promise(final Strategy<Unit> s,
			final Actor<P2<Either<P1<A>, Actor<A>>, Promise<A>>> qa) {
		this.s = s;
		actor = qa;
	}

	private static <A> Promise<A> mkPromise(final Strategy<Unit> s) {
		final Actor<P2<Either<P1<A>, Actor<A>>, Promise<A>>> q = queueActor(s,
				new Effect<P2<Either<P1<A>, Actor<A>>, Promise<A>>>() {
					public void e(
							final P2<Either<P1<A>, Actor<A>>, Promise<A>> p) {
	 ...
	 * 
	 * @param s
	 *            The strategy with which to fulfil the promise.
	 * @param a
	 *            The 1-product to evaluate concurrently.
	 * @return A promise representing the future result of evaluating the given
	 *         1-product.
	 */
	 ...
	 * @return A function that, given a 1-product, yields a promise of that
	 *         product's value.
	 */
	 ...
	 *            The strategy with which to fulfil the promise.
	 * @param a
	 *            The Callable to evaluate concurrently.
	 * @return A promise of a new Callable that will return the result of
	 *         calling the given Callable.
	 */
	public static <A> Promise<Callable<A>> promise(final Strategy<Unit> s,
			final Callable<A> a) {
	...
	/**
	 * Transforms any function so that it returns a promise of a value instead
	 * of an actual value. Represents the Kleisli arrow for the Promise monad.
	 * 
	 * @param s
	 *            The strategy with which to fulfil the promise.
	 * @param f
	 *            The function to turn into a promise-valued function.
	 * @return The given function transformed into a function that returns a
	 *         promise.
	 */
	public static <A, B> F<A, Promise<B>> promise(final Strategy<Unit> s,
			final F<A, B> f) {
	...
	/**
	 * Provides a promise to apply the given function to this promise's future
	 * value (covariant functor pattern).
	 * 
	 * @param f
	 *            The function to apply to this promise's future value.
	 * @return A promise representing the future result of applying the given
	 *         function to this promised value.
	 */
	...
	/**
	 * Promotes any function to a transformation between promises (covariant
	 * functor pattern).
	 * 
	 * @param f
	 *            The function to promote to a transformation between promises.
	...
	/**
	 * Turns a promise of a promise into just a promise. The join function for
	 * the Promise monad. Promise to give it a Promise of an A, and it will
	 * promise you an A in return.
	 * 
	 * @param p
	...
	/**
	 * Turns a product of a promise into just a promise. Does not block on the
	 * product by calling it, but creates a new promise with a final join.
	 * 
	 * @param s
	 *            The strategy with which to fulfil the promise.
	 * @param p
	 *            A product-1 of a promise to turn into just a promise.
	 * @return The joined promise.
	 */
	public static <A> Promise<A> join(final Strategy<Unit> s,
			final P1<Promise<A>> p) {
		return join(promise(s, p));
	}

	/**
	 * Binds the given function over this promise, with a final join. The bind
	 * function for the Promise monad.
	 * 
	 * @param f
	 *            The function to bind over this promise.
	...
	/**
	 * Performs function application within a promise (applicative functor
	 * pattern).
	 * 
	 * @param pf
	 *            The promised function to apply.
	 * @return A new promise after applying the given promised function to this
	 *         promise.
	 */
	...
	/**
	 * Binds the given function to this promise and the given promise, with a
	 * final join.
	 * 
	 * @param pb
	 *            A promise with which to bind the given function.
	 * @param f
	 *            The function to apply to the given promised values.
	 ...
	 * 
	 * @param s
	 *            The strategy with which to sequence the promises.
	 * @param as
	 *            The list of promises to transform.
	 * @return A single promise for the given List.
	 */
	public static <A> Promise<List<A>> sequence(final Strategy<Unit> s,
			final List<Promise<A>> as) {
		return join(foldRight(s, liftM2(List.<A> cons()),
				promise(s, P.p(List.<A> nil()))).f(as));
	}
	 ...
	 * 
	 * @param s
	 *            The strategy with which to sequence a given list of promises.
	 * @return A function that turns a list of promises into a single promise of
	 *         a list.
	 */
	public static <A> F<List<Promise<A>>, Promise<List<A>>> sequence(
	 ...
	 *            The strategy with which to sequence the promises.
	 * @param as
	 *            The Stream of promises to transform.
	 * @return A single promise for the given Stream.
	 */
	public static <A> Promise<Stream<A>> sequence(final Strategy<Unit> s,
			final Stream<Promise<A>> as) {
		return join(foldRightS(
				s,
				Function.<Promise<A>, P1<Promise<Stream<A>>>, Promise<Stream<A>>> curry((
						o, p) -> o.bind(a -> p._1().fmap(
						Stream.<A> cons_().f(a)))),
	 ...
	 * 
	 * @param s
	 *            The strategy with which to sequence a given Stream of
	 *            promises.
	 * @return A function that turns a list of promises into a single promise of
	 *         a Stream..
	 */
	public static <A> F<List<Promise<A>>, Promise<List<A>>> sequenceS(
			final Strategy<Unit> s) {
		return as -> sequence(s, as);
	}
	 ...
	 * 
	 * @param s
	 *            The strategy with which to traverse the promise.
	 * @param p
	 *            A product of a promise to traverse.
	 * @return A promised product.
	 */
	public static <A> Promise<P1<A>> sequence(final Strategy<Unit> s,
			final P1<Promise<A>> p) {
	 ...
	 * 
	 * @param s
	 *            The strategy with which to fold the list.
	 * @param f
	 ...
	 * 
	 * @param s
	 *            The strategy with which to fold the Stream.
	 * @param f
	 ...
	 * @return The final result after the right-fold reduction.
	 */
	public static <A, B> F<Stream<A>, Promise<B>> foldRightS(
			final Strategy<Unit> s, final F<A, F<P1<B>, B>> f, final B b) {
		return new F<Stream<A>, Promise<B>>() {
			public Promise<B> apply(final Stream<A> as) {
				return as.isEmpty() ? promise(s, P.p(b)) : liftM2(f).f(
						promise(s, P.p(as.head()))).f(
						Promise.<P1<B>> join(s,
								() -> f(as.tail()._1()).fmap(P.<B> p1())));
			}
		};
	}
	...
	/**
	 * Binds the given function across a promise of this promise (Comonad
	 * pattern).
	 * 
	 * @param f
	 *            A function to apply within a new promise of this promise.
	 * @return A new promise of the result of applying the given function to
	 *         this promise.
	 */
	...
	/**
	 * Applies a stream of comonadic functions to this promise, returning a
	 * stream of values.
	 * 
	 * @param fs
	 *            A stream of functions to apply to this promise.
	 * @return A stream of the results of applying the given stream of functions
	 *         to this promise.
	 */
	public <B> Stream<B> sequenceW(final Stream<F<Promise<A>, B>> fs) {
		return fs.isEmpty() ? Stream.<B> nil() : Stream.cons(fs.head().f(this),
				new P1<Stream<B>>() {
					public Stream<B> _1() {
