...
public abstract class Trampoline<A> {
	...
	// continuation. Subcomputations are only
	// allowed to be Normal, so all of the continuations accumulate on the
	// right.
	private static final class Codense<A> extends Trampoline<A> {
		...
		// The monadic bind constructs a new Codense whose subcomputation is
		// still `sub`, and Kleisli-composes the
		// continuations.
		public <B> Trampoline<B> bind(final F<A, Trampoline<B>> f) {
		...
		// If that computation is done, its result
		// gets shifted into the continuation.
		public Either<P1<Trampoline<A>>, A> resume() {
			return left(sub.resume().either(
					
					new F<P1<Trampoline<Object>>, P1<Trampoline<A>>>() {
						public P1<Trampoline<A>> f(
								final P1<Trampoline<Object>> p) {
							return p.map(new F<Trampoline<Object>, Trampoline<A>>() {
								public Trampoline<A> f(
										final Trampoline<Object> ot) {
									return ot
											.fold(new F<Normal<Object>, Trampoline<A>>() {
												public Trampoline<A> f(
														final Normal<Object> o) {
													return o.foldNormal(
															new F<Object, Trampoline<A>>() {
																public Trampoline<A> f(
																		final Object o) {
																	return cont
																			.f(o);
	...
	private static final class Suspend<A> extends Normal<A> {
		...
		public <R> R foldNormal(final F<A, R> pure,
				final F<P1<Trampoline<A>>, R> k) {
	...
	protected static <A, B> Codense<B> codense(final Normal<A> a,
			final F<A, Trampoline<B>> k) {
	...
	protected abstract <R> R fold(final F<Normal<A>, R> n,
	 ...
	 * 
	 * @param f
	 *            A function that constructs a trampoline from the result of
	 *            this trampoline.
	 ...
	 * 
	 * @param f
	 *            A function that gets applied to the result of this trampoline.
	 * @return A new trampoline that runs this trampoline, then applies the
	 *         given function to the result.
	 */
	...
	/**
	 * Performs function application within a Trampoline (applicative functor
	 * pattern).
	 * 
	 * @param lf
	 *            A Trampoline resulting in the function to apply.
	 * @return A new Trampoline after applying the given function through this
	 *         Trampoline.
	 */
	...
	/**
	 * Binds the given function across the result of this Trampoline and the
	 * given Trampoline.
	 * 
	 * @param lb
	 *            A given Trampoline to bind the given function with.
	 * @param f
	 *            The function to combine the results of this Trampoline and the
	 *            given Trampoline.
	 * @return A new Trampoline combining the results of the two trampolines
	 *         with the given function.
	 */
	public final <B, C> Trampoline<C> bind(final Trampoline<B> lb,
			final F<A, F<B, C>> f) {
	 ...
	 * 
	 * @param f
	 *            The function to promote to a function on Trampolines.
	 * @return The given function, promoted to operate on Trampolines.
	 */
	public static <A, B, C> F<Trampoline<A>, F<Trampoline<B>, Trampoline<C>>> liftM2(
			final F<A, F<B, C>> f) {
	 ...
	 * @return A new trampoline that runs this trampoline and the given
	 *         trampoline simultaneously.
	 */
	@SuppressWarnings("LoopStatementThatDoesntLoop")
	public <B, C> Trampoline<C> zipWith(final Trampoline<B> b,
			final F2<A, B, C> f) {
		...
		for (final P1<Trampoline<A>> x : ea.left()) {
			for (final P1<Trampoline<B>> y : eb.left()) {
				return suspend(P1.bind(x, y,
						new F2<Trampoline<A>, Trampoline<B>, Trampoline<C>>() {
							public Trampoline<C> f(final Trampoline<A> ta,
									final Trampoline<B> tb) {
		...
		throw Bottom
				.error("Match error: Trampoline is neither done nor suspended.");
	}
}
