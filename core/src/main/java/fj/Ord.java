...
public final class Ord<A> {
	 ...
	 * 
	 * @param oa
	 *            Order across the left side of {@link Either}.
	 * @param ob
	 *            Order across the right side of {@link Either}.
	 ...
	 * 
	 * @param oa
	 *            Order across the failing side of {@link Validation}.
	 * @param ob
	 *            Order across the succeeding side of {@link Validation}.
	 * @return An order instance for the {@link Validation} type.
	 */
	public static <A, B> Ord<Validation<A, B>> validationOrd(final Ord<A> oa,
			final Ord<B> ob) {
			...
			public F<Array<A>, Ordering> f(final Array<A> a1) {
				return new F<Array<A>, Ordering>() {
					public Ordering f(final Array<A> a2) {
						...
						return i == a1.length() ? i == a2.length() ? Ordering.EQ
								: Ordering.LT
	...
	/**
	 * An order instance for a product-2, with the first factor considered most
	 * significant.
	 * 
	 * @param oa
	 ...
	 * @return An order instance for a product-2, with the first factor
	...
	/**
	 * An order instance for a product-3, with the first factor considered most
	 * significant.
	 * 
	 * @param oa
	 *            An order instance for the first factor.
	 ...
	 * @return An order instance for a product-3, with the first factor
	 *         considered most significant.
	 */
	 ...
	 */
	public static final Ord<Natural> naturalOrd = bigintOrd
	 ...
	 * order and equality, thus objects returning the same hashCode are
	 * considered to be equals (check {@link #hashEqualsOrd()} for an additional
	 * check on {@link Object#equals(Object)}).
	 * 
	...
	public static <A> Ord<A> hashEqualsOrd() {
		return Ord.<A> ord(a -> a2 -> {
			final int x = a.hashCode() - a2.hashCode();
			return x < 0 ? Ordering.LT : x == 0 && a.equals(a2) ? Ordering.EQ
					: Ordering.GT;
		});
	}
