...
/**
 * A parser is a function that takes some input (I) and produces either an error
 * (E) or a parse result (A) and the remainder of the input.
 *
...
public final class Parser<I, A, E> {
	...
	/**
	 * Returns a parser that fails with the given error if the result value does
	 * not meet the given predicate.
	 *
	 * @param f
	 *            The predicate to filter on.
	 * @param e
	 *            The error to in the event that the predicate is not met.
	 * @return A parser that fails with the given error if the result value does
	 *         not meet the given predicate.
	 */
	public Parser<I, A, E> filter(final F<A, Boolean> f, final E e) {
		return parser(i -> parse(i).bind(
				r -> {
					final A v = r.value();
					return f.f(v) ? Validation
							.<E, Result<I, A>> success(result(r.rest(), v))
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C> Parser<I, C, E> bind(final Parser<I, B, E> pb,
			final F<A, F<B, C>> f) {
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C, D> Parser<I, D, E> bind(final Parser<I, B, E> pb,
			final Parser<I, C, E> pc, final F<A, F<B, F<C, D>>> f) {
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C, D, E$> Parser<I, E$, E> bind(final Parser<I, B, E> pb,
			final Parser<I, C, E> pc, final Parser<I, D, E> pd,
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C, D, E$, F$> Parser<I, F$, E> bind(final Parser<I, B, E> pb,
			final Parser<I, C, E> pc, final Parser<I, D, E> pd,
			final Parser<I, E$, E> pe, final F<A, F<B, F<C, F<D, F<E$, F$>>>>> f) {
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C, D, E$, F$, G> Parser<I, G, E> bind(final Parser<I, B, E> pb,
			final Parser<I, C, E> pc, final Parser<I, D, E> pd,
			final Parser<I, E$, E> pe, final Parser<I, F$, E> pf,
	 ...
	 * @return A new parser after performing the map, then final join.
	 */
	public <B, C, D, E$, F$, G, H> Parser<I, H, E> bind(
			final Parser<I, B, E> pb, final Parser<I, C, E> pc,
	...
	/**
	 * Returns a parser that tries this parser and if it fails, then tries the
	 * given parser.
	 *
	 * @param alt
	 *            The parser to try if this parser fails.
	 * @return A parser that tries this parser and if it fails, then tries the
	 *         given parser.
	 */
	public Parser<I, A, E> or(final P1<Parser<I, A, E>> alt) {
		return parser(i -> parse(i).f().sequence(alt._1().parse(i)));
	}
	...
	/**
	 * Returns a parser that tries this parser and if it fails, then tries the
	 * given parser. If both parsers fail, then append their errors with the
	 * given semigroup.
	 *
	 * @param alt
	 *            The parser to try if this parser fails.
	...
	public Parser<I, A, E> or(final P1<Parser<I, A, E>> alt,
			final Semigroup<E> s) {
		return parser(i -> parse(i).f().bind(
				e -> alt._1().parse(i).f().map(s.sum(e))));
	}
	...
	/**
	 * Returns a parser that negates this parser. If this parser succeeds, then
	 * the returned parser fails and vice versa.
	 *
	 * @param e
	...
	public Parser<I, Unit, E> not(final P1<E> e) {
		return parser(i -> parse(i).isFail() ? Validation
				.<E, Result<I, Unit>> success(result(i, unit())) : Validation
				.<E, Result<I, Unit>> fail(e._1()));
	}
	...
	/**
	 * Returns a parser that repeats application of this parser zero or many
	 * times.
	 *
	 * @return A parser that repeats application of this parser zero or many
	 *         times.
	 */
	...
	/**
	 * Returns a parser that repeats application of this parser one or many
	 * times.
	 *
	 * @return A parser that repeats application of this parser one or many
	 *         times.
	 */
	 ...
	 */
	public static <I, A, E> Parser<I, A, E> parser(
			final F<I, Validation<E, Result<I, A>>> f) {
		return new Parser<I, A, E>(f);
	}

	/**
	 * Constructs a parser that always returns the given value. The unital for a
	 * parser.
	 *
	 * @param a
	 ...
	 */
	public static <I, A, E> Parser<I, List<A>, E> sequence(
			final List<Parser<I, A, E>> ps) {
		return ps.isEmpty() ? Parser.<I, List<A>, E> value(List.<A> nil()) : ps
				.head().bind(a -> sequence(ps.tail()).map(cons_(a)));
	}
	...
	public static final class StreamParser {
		...
		/**
		 * Returns a parser that produces an element from the stream if it is
		 * available and fails otherwise.
		 *
		 * @param e
		 *            The error to fail with if no element is available.
		 * @return A parser that produces an element from the stream if it is
		 *         available and fails otherwise.
		 */
		public static <I, E> Parser<Stream<I>, I, E> element(final P1<E> e) {
			return parser(is -> is.isEmpty() ? Validation
					.<E, Result<Stream<I>, I>> fail(e._1()) : Validation
					.<E, Result<Stream<I>, I>> success(result(is.tail()._1(),
							is.head())));
		}
		...
		/**
		 * Returns a parser that produces an element from the stream that
		 * satisfies the given predicate, or fails.
		 *
		 * @param missing
		 *            The error if no element is available.
		 * @param sat
		 *            The error if the element does not satisfy the predicate.
		 * @param f
		 *            The predicate that the element should satisfy.
		 * @return A parser that produces an element from the stream that
		 *         satisfies the given predicate, or fails.
		 */
		public static <I, E> Parser<Stream<I>, I, E> satisfy(
				final P1<E> missing, final F<I, E> sat, final F<I, Boolean> f) {
			return StreamParser.<I, E> element(missing).bind(
					x -> f.f(x) ? Parser.<Stream<I>, I, E> value(x) : Parser
							.<Stream<I>, I, E> fail(sat.f(x)));
		}
		 ...
		 *         satisfies the given predicate, or fails.
		 */
		public static <I, E> Parser<Stream<I>, I, E> satisfy(final E missing,
				final F<I, E> sat, final F<I, Boolean> f) {
	...
	public static final class CharsParser {
		...
		/**
		 * Returns a parser that produces a character if one is available or
		 * fails with the given error.
		 *
		 * @param e
		 *            The error to fail with if a character is unavailable.
		 * @return A parser that produces a character if one is available or
		 *         fails with the given error.
		 */
		...
		/**
		 * Returns a parser that produces the given character or fails
		 * otherwise.
		 *
		 * @param missing
		 *            The error if no character is available.
		 * @param sat
		 *            The error if the produced character is not the one given.
		 * @param c
		 *            The character to produce in the parser.
		 * @return A parser that produces the given character or fails
		 *         otherwise.
		 */
		public static <E> Parser<Stream<Character>, Character, E> character(
				final P1<E> missing, final F<Character, E> sat, final char c) {
		 ...
		 */
		public static <E> Parser<Stream<Character>, Character, E> character(
				final E missing, final F<Character, E> sat, final char c) {
			return character(p(missing), sat, c);
		}

		/**
		 * Returns a parser that produces the given number of characters, or
		 * fails with the given error.
		 *
		 * @param missing
		 *            The error if the given number of characters is
		 *            unavailable.
		 * @param n
		 *            The number of characters to produce in the parse result.
		 * @return A parser that produces the given number of characters, or
		 *         fails with the given error.
		 */
		public static <E> Parser<Stream<Character>, Stream<Character>, E> characters(
				final P1<E> missing, final int n) {
			return n <= 0 ? Parser
					.<Stream<Character>, Stream<Character>, E> value(Stream
							.<Character> nil()) : character(missing).bind(
					characters(missing, n - 1), Stream.<Character> cons_());
		}
		 ...
		 */
		public static <E> Parser<Stream<Character>, Stream<Character>, E> characters(
				final E missing, final int n) {
			return characters(p(missing), n);
		}

		/**
		 * Returns a parser that produces the given stream of characters or
		 * fails otherwise.
		 *
		 * @param missing
		 *            The error if the producing stream could not supply more
		 *            characters.
		 * @param sat
		 *            The error if a character was produced that is not the
		 *            given stream of characters.
		 * @param cs
		 *            The stream of characters to produce.
		 * @return A parser that produces the given stream of characters or
		 *         fails otherwise.
		 */
		public static <E> Parser<Stream<Character>, Stream<Character>, E> characters(
				final P1<E> missing, final F<Character, E> sat,
				final Stream<Character> cs) {
			return cs.isEmpty() ? Parser
					.<Stream<Character>, Stream<Character>, E> value(Stream
							.<Character> nil()) : character(missing, sat,
					cs.head()).bind(characters(missing, sat, cs.tail()._1()),
					Stream.<Character> cons_());
		}
		 ...
		 */
		public static <E> Parser<Stream<Character>, Stream<Character>, E> characters(
				final E missing, final F<Character, E> sat,
				final Stream<Character> cs) {
		...
		public static <E> Parser<Stream<Character>, String, E> string(
				final P1<E> missing, final F<Character, E> sat, final String s) {
		 ...
		 */
		public static <E> Parser<Stream<Character>, String, E> string(
				final E missing, final F<Character, E> sat, final String s) {
		 ...
		 *
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a digit with.
		 * @param sat
		...
		public static <E> Parser<Stream<Character>, Digit, E> digit(
				final P1<E> missing, final F<Character, E> sat) {
			return StreamParser.satisfy(missing, sat, Character::isDigit).map(
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a lower-case character with.
		 * @param sat
		 *            The error if the produced character is not a lower-case
		 *            character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a upper-case character with.
		 * @param sat
		 *            The error if the produced character is not a upper-case
		 *            character.
		 * @return A parser that produces a upper-case character.
		 * @see Character#isUpperCase(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> upper(
				final P1<E> missing, final F<Character, E> sat) {
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a defined character with.
		 * @param sat
		 *            The error if the produced character is not a defined
		 *            character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a high-surrogate character with.
		 * @param sat
		 *            The error if the produced character is not a
		 *            high-surrogate character.
		 * @return A parser that produces a high-surrogate character.
		 * @see Character#isHighSurrogate(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> highSurrogate(
				final P1<E> missing, final F<Character, E> sat) {
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce an identifier-ignorable character with.
		 * @param sat
		 *            The error if the produced character is not an
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce an ISO control character with.
		 * @param sat
		 *            The error if the produced character is not an ISO control
		 *            character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a Java identifier part character with.
		 * @param sat
		 *            The error if the produced character is not a Java
		 *            identifier part character.
		 * @return A parser that produces a Java identifier part character.
		 * @see Character#isJavaIdentifierPart(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> javaIdentifierPart(
				final P1<E> missing, final F<Character, E> sat) {
			return StreamParser.satisfy(missing, sat,
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a Java identifier start character with.
		 * @param sat
		 *            The error if the produced character is not a Java
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce an alpha character with.
		 * @param sat
		 *            The error if the produced character is not an alpha
		 *            character.
		 * @return A parser that produces an alpha character.
		 * @see Character#isLetter(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> alpha(
				final P1<E> missing, final F<Character, E> sat) {
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce an alpha-numeric character with.
		 * @param sat
		 *            The error if the produced character is not an
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a low-surrogate character with.
		 * @param sat
		 *            The error if the produced character is not a low-surrogate
		 *            character.
		...
		public static <E> Parser<Stream<Character>, Character, E> lowSurrogate(
				final P1<E> missing, final F<Character, E> sat) {
			return StreamParser
					.satisfy(missing, sat, Character::isLowSurrogate);
		}
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a mirrored character with.
		 * @param sat
		 *            The error if the produced character is not a mirrored
		 *            character.
		 * @return A parser that produces a mirrored character.
		 * @see Character#isMirrored(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> mirrored(
				final P1<E> missing, final F<Character, E> sat) {
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a space character with.
		 * @param sat
		 *            The error if the produced character is not a space
		 *            character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a title-case character with.
		 * @param sat
		 *            The error if the produced character is not a title-case
		 *            character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a unicode identifier part character with.
		 * @param sat
		 *            The error if the produced character is not a unicode
		 *            identifier part character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a unicode identifier start character with.
		 * @param sat
		 *            The error if the produced character is not a unicode
		 *            identifier start character.
		 ...
		 * @param missing
		 *            The error if there is no character on the stream to
		 *            produce a white-space character with.
		 * @param sat
		 *            The error if the produced character is not a white-space
		 *            character.
		 * @return A parser that produces a white-space character.
		 * @see Character#isWhitespace(char)
		 */
		public static <E> Parser<Stream<Character>, Character, E> whitespace(
				final P1<E> missing, final F<Character, E> sat) {
