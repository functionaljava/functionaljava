...
/**
 * An argument used in a property that may have undergone shrinking following
 * falsification.
 * 
...
public final class Arg<T> {
	 ...
	 * 
	 * @param value
	 *            The value to construct an argument with.
	 * @param shrinks
	 *            The number of shrinks to construct an argument with.
	...
	/**
	 * The rendering of an argument (uses {@link Object#toString()} for the
	 * argument value).
	 */
	public static final Show<Arg<?>> argShow = showS(arg -> anyShow().showS(
			arg.value)
			+ (arg.shrinks > 0 ? " (" + arg.shrinks + " shrink"
					+ (arg.shrinks == 1 ? "" : 's') + ')' : ""));
}
