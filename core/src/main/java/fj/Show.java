...
public final class Show<A> {
	 ...
	 * @param a
	 *            The argument to display.
	 * @return The display rendering of the given argument as a
	 *         <code>String</code>.
	 */
	...
	public Unit print(final A a) {
		...
		int c = 0;
		for (Stream<Character> cs = show(a); cs.isNotEmpty(); cs = cs.tail()
				._1()) {
			buffer[c] = cs.head();
			c++;
			if (c == 8192) {
	...
	/**
	 * Returns a show instance that uses {@link Object#toString()} to perform
	 * the display rendering.
	 * 
	 * @return A show instance that uses {@link Object#toString()} to perform
	 *         the display rendering.
	 */
	...
	public static <A> Show<Option<A>> optionShow(final Show<A> sa) {
		return new Show<Option<A>>(o -> o.isNone() ? fromString("None")
				: fromString("Some(").append(sa.f.f(o.some())).append(
						single(')')));
	}
	 ...
	 * 
	 * @param sa
	 *            Show for the left side of the {@link Either}.
	 * @param sb
	 *            Show for the right side of the {@link Either}.
	 ...
	 * 
	 * @param sa
	 *            Show for the fail side of the {@link Validation}.
	 * @param sb
	 *            Show for the success side of the {@link Validation}.
		...
		return new Show<Stream<A>>(new F<Stream<A>, Stream<Character>>() {
			public Stream<Character> f(final Stream<A> as) {
				return join(as.map(sa.show_()).intersperse(fromString(","))
						.cons(fromString("<")).snoc(p(fromString(">"))));
			}
		});
	}
	 ...
	 */
	public static <A, B> Show<P2<A, B>> p2Show(final Show<A> sa,
			final Show<B> sb) {
		return new Show<P2<A, B>>(p -> cons('(', p(sa.show(p._1()))).snoc(',')
	 ...
	 */
	public static <A, B, C, D> Show<P4<A, B, C, D>> p4Show(final Show<A> sa,
			final Show<B> sb, final Show<C> sc, final Show<D> sd) {
		return new Show<P4<A, B, C, D>>(
				new F<P4<A, B, C, D>, Stream<Character>>() {
					public Stream<Character> f(final P4<A, B, C, D> p) {
						return cons('(', p(sa.show(p._1()))).snoc(',')
								.append(sb.show(p._2())).snoc(',')
								.append(sc.show(p._3())).snoc(',')
								.append(sd.show(p._4())).snoc(')');
					}
				});
	}
	 ...
	 */
	public static <A, B, C, D, E> Show<P5<A, B, C, D, E>> p5Show(
			final Show<A> sa, final Show<B> sb, final Show<C> sc,
			final Show<D> sd, final Show<E> se) {
		return new Show<P5<A, B, C, D, E>>(p -> cons('(', p(sa.show(p._1())))
				.snoc(',').append(sb.show(p._2())).snoc(',')
				.append(sc.show(p._3())).snoc(',').append(sd.show(p._4()))
				.snoc(',').append(se.show(p._5())).snoc(')'));
	}
	 ...
	 */
	public static <A, B, C, D, E, F$> Show<P6<A, B, C, D, E, F$>> p6Show(
			final Show<A> sa, final Show<B> sb, final Show<C> sc,
			final Show<D> sd, final Show<E> se, final Show<F$> sf) {
		return new Show<P6<A, B, C, D, E, F$>>(
				new F<P6<A, B, C, D, E, F$>, Stream<Character>>() {
					public Stream<Character> f(final P6<A, B, C, D, E, F$> p) {
						return cons('(', p(sa.show(p._1()))).snoc(',')
								.append(sb.show(p._2())).snoc(',')
								.append(sc.show(p._3())).snoc(',')
								.append(sd.show(p._4())).snoc(',')
								.append(se.show(p._5())).snoc(',')
								.append(sf.show(p._6())).snoc(')');
					}
				});
	}
	 ...
	 */
	public static <A, B, C, D, E, F$, G> Show<P7<A, B, C, D, E, F$, G>> p7Show(
			final Show<A> sa, final Show<B> sb, final Show<C> sc,
			final Show<D> sd, final Show<E> se, final Show<F$> sf,
			final Show<G> sg) {
		return new Show<P7<A, B, C, D, E, F$, G>>(
				new F<P7<A, B, C, D, E, F$, G>, Stream<Character>>() {
	 ...
	 */
	public static <A, B, C, D, E, F$, G, H> Show<P8<A, B, C, D, E, F$, G, H>> p8Show(
			final Show<A> sa, final Show<B> sb, final Show<C> sc,
			final Show<D> sd, final Show<E> se, final Show<F$> sf,
			final Show<G> sg, final Show<H> sh) {
		return new Show<P8<A, B, C, D, E, F$, G, H>>(
				new F<P8<A, B, C, D, E, F$, G, H>, Stream<Character>>() {
					public Stream<Character> f(
							final P8<A, B, C, D, E, F$, G, H> p) {
						return cons('(', p(sa.show(p._1()))).snoc(',')
								.append(sb.show(p._2())).snoc(',')
								.append(sc.show(p._3())).snoc(',')
								.append(sd.show(p._4())).snoc(',')
	 ...
	 */
	public static final Show<Natural> naturalShow = bigintShow
			.comap(Natural::bigIntegerValue);
	...
	public static <A> Show<Stream<A>> unlineShow(final Show<A> sa) {
		return new Show<Stream<A>>(as -> join(as.map(sa.show_()).intersperse(
				fromString("\n"))));
	}
	 ...
	 */
	public static final Show<LazyString> lazyStringShow = show(string -> string
			.toStream());

	/**
	 * A show instance for the empty heterogeneous Stream.
	 */
	public static final Show<HList.HNil> HListShow = showS(Function
			.<HList.HNil, String> constant("Nil"));

	/**
	 * A show instance for heterogeneous Streams.
	 * 
	 * @param e
	 *            A show instance for the first element of the Stream.
	 * @param l
	 *            A show instance for the rest of the Stream.
	 * @return a show instance for heterogeneous Streams.
	 */
	public static <E, L extends HList<L>> Show<HList.HCons<E, L>> HListShow(
			final Show<E> e, final Show<L> l) {
		return show(c -> e.show(c.head()).cons('[').append(l.show(c.tail()))
				.snoc(']'));
	}
}
