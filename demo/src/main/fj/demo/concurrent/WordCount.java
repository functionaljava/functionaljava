...
/**
 * Reads words and their counts from files (
 * {@link #getWordsAndCountsFromFiles(List)} in a single thread and
 * {@link #getWordsAndCountsFromFilesInParallel(List, int)} in multiple
 * threads). The files are created initially and populated with some sample
 * content.
 * 
...
public class WordCount {
	...
	private static <K, V> Map<K, V> update(Map<K, V> map, K key,
			F<V, V> valueFunction, V initialValue) {
		V value = map.get(key);
		if (value == null) {
			value = initialValue;
		}
	...
	private static final F<String, Map<String, Integer>> fileNameToWordsAndCountsWithCharChunkIteratee = fileName -> {
		try {
			return IO
					.enumFileCharChunks(new File(fileName),
							Option.<Charset> none(), wordCountsFromCharChunks())
					.run().run();
	...
	private static final F<String, Map<String, Integer>> fileNameToWordsAndCountsWithCharChunk2Iteratee = fileName -> {
		try {
			return IO
					.enumFileChars(new File(fileName), Option.<Charset> none(),
							wordCountsFromChars()).run().run();
		} catch (final IOException e) {
			throw new RuntimeException(e);
		}
	...
	public static final <E> IterV<char[], Map<String, Integer>> wordCountsFromCharChunks() {
		final F<P2<StringBuilder, Map<String, Integer>>, F<Input<char[]>, IterV<char[], Map<String, Integer>>>> step = new F<P2<StringBuilder, Map<String, Integer>>, F<Input<char[]>, IterV<char[], Map<String, Integer>>>>() {
			...
			@Override
			public F<Input<char[]>, IterV<char[], Map<String, Integer>>> apply(
					final P2<StringBuilder, Map<String, Integer>> acc) {
				final P1<IterV<char[], Map<String, Integer>>> empty = new P1<IterV<char[], Map<String, Integer>>>() {
					@Override
						...
						return e -> {
							...
							for (char c : e) {
								if (Character.isWhitespace(c)) {
									if (sb.length() > 0) {
										map = update(map, sb.toString(),
												addOne, Integer.valueOf(0));
				...
				final P1<IterV<char[], Map<String, Integer>>> eof = new P1<IterV<char[], Map<String, Integer>>>() {
					@Override
					public IterV<char[], Map<String, Integer>> _1() {
						final StringBuilder sb = acc._1();
						if (sb.length() > 0) {
							final Map<String, Integer> map = update(acc._2(),
									sb.toString(), addOne, Integer.valueOf(0));
		...
		return IterV.cont(step.f(P.p(new StringBuilder(),
	...
	public static final <E> IterV<Character, Map<String, Integer>> wordCountsFromChars() {
		final F<P2<StringBuilder, Map<String, Integer>>, F<Input<Character>, IterV<Character, Map<String, Integer>>>> step = new F<P2<StringBuilder, Map<String, Integer>>, F<Input<Character>, IterV<Character, Map<String, Integer>>>>() {
			final F<P2<StringBuilder, Map<String, Integer>>, F<Input<Character>, IterV<Character, Map<String, Integer>>>> step = this;

			@Override
			public F<Input<Character>, IterV<Character, Map<String, Integer>>> apply(
					final P2<StringBuilder, Map<String, Integer>> acc) {
				final P1<IterV<Character, Map<String, Integer>>> empty = new P1<IterV<Character, Map<String, Integer>>>() {
					@Override
					public IterV<Character, Map<String, Integer>> _1() {
						return IterV.cont(step.f(acc));
					}
				};
				final P1<F<Character, IterV<Character, Map<String, Integer>>>> el = new P1<F<Character, IterV<Character, Map<String, Integer>>>>() {
					@Override
					public F<Character, IterV<Character, Map<String, Integer>>> _1() {
						return e -> {
							if (Character.isWhitespace(e.charValue())) {
								final StringBuilder sb = acc._1();
								if (sb.length() > 0) {
									final Map<String, Integer> map = update(
											acc._2(), sb.toString(), addOne,
											Integer.valueOf(0));
									return IterV.cont(step.f(P.p(
											new StringBuilder(), map)));
								} else {
									// another whitespace char, no word to
									// push to the map
									return IterV.cont(step.f(acc));
								}
							} else {
				...
				final P1<IterV<Character, Map<String, Integer>>> eof = new P1<IterV<Character, Map<String, Integer>>>() {
					@Override
					public IterV<Character, Map<String, Integer>> _1() {
						final StringBuilder sb = acc._1();
						if (sb.length() > 0) {
	...
	public static void main(String[] args) throws IOException {
		...
		final P2<List<String>, Map<String, Integer>> result = writeSampleFiles(
				numFiles, numSharedWords);
		final List<String> fileNames = result._1();
		final Map<String, Integer> expectedWordsAndCounts = result._2();
		long avgSize = fileNames.foldLeft((a, file) -> a.longValue()
				+ new File(file).length(), 0L)
				/ fileNames.length();
		System.out.println("Processing " + numFiles + " files with ~"
				+ numSharedWords + " words and an avg size of " + avgSize
				+ " bytes.");
		...
		for (int i = 0; i < 1; i++) {
			...
			getWordsAndCountsFromFilesWithIteratee(fileNames.take(1),
					fileNameToWordsAndCountsWithCharIteratee);
			getWordsAndCountsFromFilesWithIteratee(fileNames.take(1),
					fileNameToWordsAndCountsWithCharChunkIteratee);
			getWordsAndCountsFromFilesWithIteratee(fileNames.take(1),
					fileNameToWordsAndCountsWithCharChunk2Iteratee);
			getWordsAndCountsFromFilesWithIteratee(fileNames.take(1),
					fileNameToWordsAndCountsWithCharChunk2Iteratee);
			// getWordsAndCountsFromFilesInParallel(fileNames.take(1),
			// fileNameToWordsAndCounts, 8);
			getWordsAndCountsFromFilesInParallel(fileNames.take(1),
					fileNameToWordsAndCountsWithCharIteratee, 8);
			getWordsAndCountsFromFilesInParallel(fileNames.take(1),
					fileNameToWordsAndCountsWithCharChunkIteratee, 8);
		}
		...
		// System.out.println("Getting word counts in 1 thread took " +
		// (System.currentTimeMillis() - start) + " ms.");
		// assertTrue(wordsAndCountsFromFiles != null);
		// assertTrue(wordsAndCountsFromFiles.size() == numFiles +
		// numSharedWords);
		...
		wordsAndCountsFromFiles = getWordsAndCountsFromFilesWithIteratee(
				fileNames, fileNameToWordsAndCountsWithCharIteratee);
		System.out
				.println("Getting word counts in 1 thread using char iteratee took "
						+ (System.currentTimeMillis() - start) + " ms.");
		assertTrue(wordsAndCountsFromFiles != null);
		...
		start = System.currentTimeMillis();
		wordsAndCountsFromFiles = getWordsAndCountsFromFilesWithIteratee(
				fileNames, fileNameToWordsAndCountsWithCharChunkIteratee);
		System.out
				.println("Getting word counts in 1 thread using char chunk iteratee took "
						+ (System.currentTimeMillis() - start) + " ms.");
		assertTrue(wordsAndCountsFromFiles != null);
		assertEquals(wordsAndCountsFromFiles.size(), numFiles + numSharedWords);
		assertEquals(wordsAndCountsFromFiles, expectedWordsAndCounts);
		...
		start = System.currentTimeMillis();
		wordsAndCountsFromFiles = getWordsAndCountsFromFilesWithIteratee(
				fileNames, fileNameToWordsAndCountsWithCharChunk2Iteratee);
		System.out
				.println("Getting word counts in 1 thread using char chunk2 iteratee took "
						+ (System.currentTimeMillis() - start) + " ms.");
		assertTrue(wordsAndCountsFromFiles != null);
		assertEquals(wordsAndCountsFromFiles.size(), numFiles + numSharedWords);
		assertEquals(wordsAndCountsFromFiles, expectedWordsAndCounts);
		...
		// getWordsAndCountsFromFilesInParallel(fileNames,
		// fileNameToWordsAndCounts, 8);
		// System.out.println("Getting word counts in 8 threads took " +
		// (System.currentTimeMillis() - start) + " ms.");
		// assertTrue(wordsAndCountsFromFiles != null);
		// assertEquals(wordsAndCountsFromFiles.size(), numFiles +
		// numSharedWords);
		// assertEquals(wordsAndCountsFromFiles, expectedWordsAndCounts);

		start = System.currentTimeMillis();
		wordsAndCountsFromFiles = getWordsAndCountsFromFilesInParallel(
				fileNames, fileNameToWordsAndCountsWithCharIteratee, 32);
		System.out
				.println("Getting word counts in 32 threads with char iteratee took "
						+ (System.currentTimeMillis() - start) + " ms.");
		assertTrue(wordsAndCountsFromFiles != null);
		assertEquals(wordsAndCountsFromFiles.size(), numFiles + numSharedWords);
		assertEquals(wordsAndCountsFromFiles, expectedWordsAndCounts);
		...
		start = System.currentTimeMillis();
		wordsAndCountsFromFiles = getWordsAndCountsFromFilesInParallel(
				fileNames, fileNameToWordsAndCountsWithCharChunkIteratee, 32);
		System.out
				.println("Getting word counts in 32 threads with char chunk iteratee took "
						+ (System.currentTimeMillis() - start) + " ms.");
		assertTrue(wordsAndCountsFromFiles != null);
		assertEquals(wordsAndCountsFromFiles.size(), numFiles + numSharedWords);
		assertEquals(wordsAndCountsFromFiles, expectedWordsAndCounts);
	...
	private static void print(Map<String, Integer> wordsAndCountsFromFiles) {
		for (final Map.Entry<String, Integer> entry : wordsAndCountsFromFiles
				.entrySet()) {
	...
	private static P2<List<String>, Map<String, Integer>> writeSampleFiles(
		...
		for (int i = 0; i < numFiles; i++) {
			final File file = File.createTempFile("wordcount-" + i + "-",
					".txt");
	...
	public static Map<String, Integer> getWordsAndCountsFromFilesWithIteratee(
			final List<String> fileNames,
			final F<String, Map<String, Integer>> fileNameToWordsAndCountsWithIteratee) {
		final List<Map<String, Integer>> maps = fileNames
				.map(fileNameToWordsAndCountsWithIteratee);
		return maps.foldLeft((a, b) -> plus(a, b),
				(Map<String, Integer>) new HashMap<String, Integer>());
	}

	public static Map<String, Integer> getWordsAndCountsFromFilesInParallel(
			final List<String> fileNames,
			final F<String, Map<String, Integer>> fileNameToWordsAndCounts,
			int numThreads) {
		...
		final Map<String, Integer> result = getWordsAndCountsFromFiles(
				fileNames, fileNameToWordsAndCounts, m).claim();
	...
	public static Promise<Map<String, Integer>> getWordsAndCountsFromFiles(
			...
			final F<String, Map<String, Integer>> fileNameToWordsAndCounts,
			final ParModule m) {
		final F<Map<String, Integer>, F<Map<String, Integer>, Map<String, Integer>>> MapSum = a -> b -> plus(
				a, b);
	...
	private static Map<String, Integer> plus(Map<String, Integer> a,
		...
		for (Map.Entry<String, Integer> entry : b.entrySet()) {
			...
			result.put(
					entry.getKey(),
					num != null ? num.intValue() + entry.getValue() : entry
							.getValue());
		}
		return result;
	}
	...
	static void assertEquals(Object actual, Object expected) {
		if (!expected.equals(actual)) {
			throw new IllegalArgumentException("Not equals. Expected: "
