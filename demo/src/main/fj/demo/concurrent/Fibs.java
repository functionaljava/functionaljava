...
public class Fibs {
	...
	public static void main(final String[] args) {
		...
		final Actor<List<Integer>> out = actor(su, new Effect<List<Integer>>() {
			public void e(final List<Integer> fs) {
				for (final P2<Integer, Integer> p : fs.zipIndex()) {
					System.out.println(MessageFormat.format("n={0} => {1}",
							p._2(), p._1()));
				}
				pool.shutdown();
			}
		...
		final F<Integer, Promise<Integer>> fib = new F<Integer, Promise<Integer>>() {
			public Promise<Integer> apply(final Integer n) {
				return n < CUTOFF ? promise(su, P.p(seqFib(n))) : apply(n - 1)
						.bind(apply(n - 2), add);
			}
		};
		...
		join(
				su,
				spi.parMap(fib, range(0, 46)).map(
						Promise.<Integer> sequence(su))).to(out);
	}
