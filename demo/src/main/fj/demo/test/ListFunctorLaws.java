...
public final class ListFunctorLaws {
	final Property identity = property(arbList(arbString),
			x -> prop(listEqual(stringEqual).eq(x, x.map(Function.identity()))));

	final Property composition = property(
			arbF(coarbInteger, arbString),
			arbF(coarbLong, arbInteger),
			arbList(arbLong),
			new F3<F<Integer, String>, F<Long, Integer>, List<Long>, Property>() {
				public Property f(final F<Integer, String> f,
						final F<Long, Integer> g, final List<Long> x) {
	...
	public static void main(final String[] args) {
		check(ListFunctorLaws.class).foreach(
				new Effect<P2<String, CheckResult>>() {
