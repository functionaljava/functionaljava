...
public final class Reflect {
	...
	final Property p1 = property(arbInteger, arbInteger,
			(a, b) -> prop(a + b == b + a));

	@Name("Natural Integer Addition yields Natural Integer")
	@Category("Fails for demo")
	final Property p2 = property(arbIntegerBoundaries, arbIntegerBoundaries,
			shrinkInteger, shrinkInteger, (a, b) -> bool(a > 0 && b > 0)
					.implies(a + b > 0));

	@Category("Passes for demo")
	final Property p3 = property(arbStringBuilder,
			sb -> prop(stringBuilderEqual.eq(sb, sb.reverse().reverse())));

	@Category("Passes for demo")
	final Property p4 = property(arbCharacter, c -> prop(stringBuilderEqual.eq(
			new StringBuilder().append(c), new StringBuilder().append(c)
					.reverse())));

	@Category("Passes for demo")
	@CheckParams(minSuccessful = 750, maxSize = 40)
	final Property p5 = property(arbStringBuilder, arbStringBuilder,
			(x, y) -> {
				// copy the string builders before performing updates on x
				// and y.
			final StringBuilder xx = new StringBuilder(x);
			final StringBuilder yy = new StringBuilder(y);
			return prop(stringBuilderEqual.eq(x.append(y).reverse(), yy
					.reverse().append(xx.reverse())));
		});

	@Name("Triangulation")
	@Category("Fails for demo")
	final Property p6 = property(arbInteger,
			a -> prop(Triangulation.isPositive(a) == (a != 0 && !Triangulation
					.isNegative(a))));
	...
	/*
	 * OK, passed 20 tests. (Integer Addition Commutes) Falsified after 22
	 * passed tests with arguments: [2147483646,13] (Natural Integer Addition
	 * yields Natural Integer) OK, passed 500 tests. (p3) OK, passed 500 tests.
	 * (p4) OK, passed 750 tests. (p5) Falsified after 0 passed tests with
	 * argument: 0 (Triangulation)
	 * ----------------------------------------------
	 * ---------------------------------- OK, passed 20 tests. (Integer Addition
	 * Commutes) OK, passed 500 tests. (p3) OK, passed 500 tests. (p4) OK,
	 * passed 750 tests. (p5)
	 * ----------------------------------------------------
	 * ---------------------------- Falsified after 0 passed tests with
	 * arguments: [2,2147483646] (Natural Integer Addition yields Natural
	 * Integer) Falsified after 0 passed tests with argument: 0 (Triangulation)
	 * --
	 * ------------------------------------------------------------------------
	 * ------ OK, passed 20 tests. (Integer Addition Commutes) Falsified after 0
	 * passed tests with arguments: [2147483647,1] (Natural Integer Addition
	 * yields Natural Integer) OK, passed 500 tests. (p3) OK, passed 500 tests.
	 * (p4) OK, passed 750 tests. (p5) Falsified after 0 passed tests with
	 * arguments: 0 (Triangulation)
	 * ----------------------------------------------
	 * ----------------------------------
	 */
