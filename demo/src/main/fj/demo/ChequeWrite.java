...
public final class ChequeWrite {
	...
	static List<Integer> toZero(final int from) {
		return unfold(i -> i < 0 ? Option.<P2<Integer, Integer>> none()
				: some(p(i, i - 1)), from);
	}
	...
	static List<Character> show(final char c) {
		return stringShow.show(
				list("zero", "one", "two", "three", "four", "five", "six",
						"seven", "eight", "nine").index(c - '0')).toList();
	}
	...
	static List<Character> show(final List<Character> cs) {
		if (cs.isEmpty())
			...
			if (d1r.isEmpty())
				...
				return d2r.isEmpty() ? d1 == '0' ? show(d2)
						: d1 == '1' ? stringShow.showl(list("ten", "eleven",
								"twelve", "thirteen", "fourteen", "fifteen",
								"sixteen", "seventeen", "eighteen", "nineteen")
								.index(d2 - '0')) : stringShow.showl(
								list("twenty", "thirty", "forty", "fifty",
										"sixty", "seventy", "eighty", "ninety")
										.index(d1 - '0' - 2)).append(
								d2 == '0' ? List.<Character> nil() : show(d2)
										.cons('-')) : d1 == '0' && d2 == '0'
						&& d2r.head() == '0' ? List.<Character> nil()
						: d1 == '0' ? show(list(d2, d2r.head())) : d2 == '0'
								&& d2r.head() == '0' ? show(d1).append(
								stringShow.showl(" hundred")) : show(d1)
								.append(stringShow.showl(" hundred and "))
								.append(show(list(d2, d2r.head())));
			}
		}
	}
	...
	static <A> List<P2<List<A>, Integer>> split(final List<A> as) {
		...
		final List<List<A>> ds = as.zip(toZero(len - 1)).foldRight(
				(ki, z) -> ki._2() % 3 == 0 ? z.conss(single(ki._1())) : z
						.tail().conss(z.head().cons(ki._1())), List.nil());
	...
	static List<Character> illion(final int i) {
		return stringShow.show(
				stream("thousand", "million", "billion", "trillion",
						...
						"vigintillion").append(
						Stream.repeat("<unsupported ?illion>")).index(i))
				.toList();
	}

	static boolean and(final List<Character> cs) {
		return cs.length() == 3 && cs.head() == '0'
				&& (cs.tail().head() != '0' || cs.tail().tail().head() != '0');
	}
	...
	static final F<List<Character>, List<Character>> cents = a -> {
		...
		return n == 0 ? fromString("zero cents") : show(
				list(a.head(), n == 1 ? '0' : a.tail().head())).append(
				fromString(eq(a, list('0', '1')) ? " cent" : " cents"));
	};

	public static List<Character> write(final List<Character> cs) {
		final F<List<Character>, List<Character>> dropNonDigit = cs2 -> cs2
				.filter(Characters.isDigit);
		final P2<List<Character>, List<Character>> x = cs
				.dropWhile(charEqual.eq('0')).breakk(charEqual.eq('.'))
				.map1(dropNonDigit).map1(dollars).map2(dropNonDigit)
	...
	public static void tests() {
		// show
		for (final P2<String, String> t : list(p("1", "one"), p("10", "ten"),
				p("15", "fifteen"), p("40", "forty"), p("45", "forty-five"))) {
